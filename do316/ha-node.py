...
#
# CHANGELOG
# * Tue May 10 2022 Maria Ordonez <mordonez@redhat.com>
#   - original code

"""
Grading module for DO316 ha-node guided exercise.
This module either does start or finish for the ha-node guided exercise.
"""
...
class HANode(OpenShift):
    """
    HA Node GE script for DO316
    """
    ...
    def start(self):
        ...
        items = [
            ...
            {
                "label": "Creating the web1 virtual machine (be patient)",
                "task": self.run_playbook,
                "playbook": f"ansible/{self.__LAB__}/web1.yml",
                "fatal": True,
            },
            {
                "label": "Creating the web2 virtual machine (be patient)",
                "task": self.run_playbook,
                "playbook": f"ansible/{self.__LAB__}/web2.yml",
                "fatal": True,
            },
            {
                "label": "Creating the web service and route",
                "task": self.run_playbook,
                "playbook": f"ansible/{self.__LAB__}/svc.yml",
                "fatal": True,
            },
            {
    ...
    def finish(self):
        ...
        items = [
            ...
            {
                "label": "Confirming the node is active",
                "task": self._confirm_host,
                "name": "worker01",
                "fatal": True,
            },
            {
    ...
    def _check_ge_namespace(self, item):
        ...
        item["failed"] = False
        if self.resource_exists(body["apiVersion"], body["kind"], (NAMESPACE), ""):
            item["failed"] = True
            item["msgs"] = [
                {
                    "text": f"The {NAMESPACE} namespace already exists, please delete it or run 'lab finish {self.__LAB__}' before starting this GE"
                }
            ]
        #        else:
        #            self.oc_client.resources.get(api_version=body["apiVersion"], kind=body["kind"]).create(body=body, namespace=None)
        return item["failed"]
    ...
    def _confirm_host(self, item):
        item["failed"] = False
        node = item["name"]
        resource = self.resource_get("v1", "Node", node, "")
        try:
            status = resource.status.conditions[3].get("type")
        except Exception:
            item["failed"] = True
            item["msgs"] = [{"text": f"The {node} node is not running."}]
            return

        if status != "Ready":
            item["failed"] = True
            item["msgs"] = [{"text": f"The {node} node is not ready, status: {status}"}]
